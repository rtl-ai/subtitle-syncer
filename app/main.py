...
def _snapshot_event(job: JobState) -> JobEvent:
    return JobEvent(
...
def _append_event(job: JobState, *, message: Optional[str] = None, progress: Optional[float] = None) -> None:
    event = JobEvent(
        timestamp=datetime.utcnow(),
        status=job.status,
        current_step=job.current_step,
...
async def _register_job(job: JobState) -> None:
    async with registry_lock:
        if not job.events:
            job.events.append(_snapshot_event(job))
...
async def _update_job(job_id: str, **changes: object) -> JobState:
    async with registry_lock:
        ...
        if any(key in changes for key in {"status", "message", "current_step", "progress"}):
            _append_event(job)
        return job
...
async def _append_log(job_id: str, name: str, entry: CommandResult) -> None:
    async with registry_lock:
        ...
        if job:
            ...
            _append_event(job, message=f"Captured {name} logs")
    ...
    try:
        ...
        if not encoding:
            ...
            await _update_job(
                ...
                current_step="Encoding detected",
                progress=0.35,
            )
        else:
            await _update_job(
                job_id,
                message=f"Using provided encoding: {encoding}",
                current_step="Encoding confirmed",
                progress=0.35,
            )
        ...
        await _update_job(
            ...
            current_step="Normalized subtitles",
            progress=0.55,
        )
        ...
        await _update_job(
            ...
            current_step="Aligned subtitles",
            progress=0.9,
        )
